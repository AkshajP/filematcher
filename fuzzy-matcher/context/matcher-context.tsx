...
type MatcherAction =
  ...
  | { type: 'IMPORT_MAPPINGS'; payload: { mappings: MatchedPair[]; newReferences: FileReference[]; usedPaths: Set<string>; folderName?: string } }
...
function matcherReducer(state: MatcherState, action: MatcherAction): MatcherState {
  switch (action.type) {
    ...
    case 'SELECT_ALL_REFERENCES': {
      ...
      let newSelected: Array<{ item: FileReference; order: number }> = [];
    ...
    case 'IMPORT_MAPPINGS': {
        ...
        // Find which references are now mapped by this import
        const mappedReferences = new Set(mappings.map(m => m.reference));
        
        // Remove newly mapped references from current unmatched list
        const newUnmatched = state.unmatchedReferences.filter(ref => !mappedReferences.has(ref.description));
        
        // Add any new references that aren't already mapped
        const additionalUnmatched = newReferences.filter(ref => !mappedReferences.has(ref.description));
        const finalUnmatched = [...newUnmatched, ...additionalUnmatched];
        
        // Set next reference if none currently selected
        const nextReference = finalUnmatched.length > 0 ? finalUnmatched[0] : null;
        
        return {
          ...state,
          fileReferences: allReferences,
          unmatchedReferences: finalUnmatched,
...
export function MatcherProvider({ children }: { children: ReactNode }) {
  ...
  const actions: MatcherActions = {
    selectReference: useCallback((reference: FileReference) => {
      dispatch({ type: 'SELECT_REFERENCE', payload: reference });
    }, []),

    toggleReferenceSelection: useCallback((reference: FileReference) => {
