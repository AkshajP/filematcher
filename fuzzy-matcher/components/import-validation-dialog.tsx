...
export function ImportValidationDialog({
  ...
  const [options, setOptions] = useState<ImportOptions>({
    ...
    createReferencesForNewFiles: true
  });
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                ...
                <label htmlFor="exactMatches" className="text-sm">
                  Import exact matches ({validationSummary.exactMatches} files)
                </label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <label htmlFor="potentialMatches" className="text-sm">
                  Import potential matches with path corrections ({potentialMatches.length} files)
                </label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="createReferences"
                  checked={options.createReferencesForNewFiles}
                  onCheckedChange={(checked) => 
                    setOptions(prev => ({ ...prev, createReferencesForNewFiles: !!checked }))
                  }
                  disabled={newFiles.length === 0}
                />
                <label htmlFor="createReferences" className="text-sm">
                  Create references for new files ({newFiles.length} files)
                </label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <label htmlFor="missingAsSkipped" className="text-sm">
                  Skip references for missing files ({missingFiles.length} files)
                </label>
              </div>
            </div>
          </div>
