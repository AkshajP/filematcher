// fuzzy-matcher.js - Fuzzy Matching Algorithms (Reharmonized)
 ...
 * @param {string} filePath The full file path to score.
 ...
 * This is the "secret sauce" that makes the auto-matcher much smarter.
...
/**
 * The application's core, non-learning-enhanced search function.
 * It uses original, cleaned, and key-term based matching to find the best possible score.
 * This function will be wrapped by the learning engine.
 * @param {string} searchTerm - The term to search for.
 * @returns {Array<Object>} A sorted array of match objects.
 */
function baseSearchMatches(searchTerm) {
    const cleanedSearchTerm = cleanFileName(searchTerm);
    const keyTermsSearch = extractKeyTerms(searchTerm);
    const availableFilePaths = window.filePaths.filter(path => !window.usedFilePaths.has(path));
